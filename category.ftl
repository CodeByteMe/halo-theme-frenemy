...
<@layout title="分类：${category.name} - ${options.blog_title!}" keywords="${options.seo_keywords!}" description="${options.seo_description!}" bodyClass="tag-template">
    <section class="site-hero responsive-title-img" style="background-image: url('${settings.index_cover!'${static}/assets/images/hero-background.jpg'}');">
        <div class="container">
            <div class="hero-content">
                <h1 class="site-name">分类：${category.name!}</h1>
                <h2 class="site-description">${category.description!}</h2>
            </div>
        </div>
    </section>
                    ...
                    <nav aria-label="文章分页">
                        <ul class="pagination">
                            <#if posts.hasPrevious()>
                                <#if posts.number == 1>
                                    <li class="page-item">
                                        <a class="page-link" href="${context!}/categories/${category.slugName!}" aria-label="上一页">
                                ...
                                <#else>
                                    <li class="page-item">
                                        <a class="page-link" href="${context!}/categories/${category.slugName!}/page/${posts.number}" aria-label="上一页">
                                          <span aria-hidden="true">
                                            <i class="fas fa-angle-left"></i>
                                          </span>
                                        </a>
                            ...
                            <#list rainbow as r>
                                ...
                                <#else>
                                    <li class="page-item"><a class="page-link" href="${context!}/categories/${category.slugName!}/page/${r}">${r}</a></li>
                                </#if>
                            </#list>
                            <#if posts.hasNext()>
                                <li class="page-item">
                                    <a class="page-link" href="${context!}/categories/${category.slugName!}/page/${posts.number+2}/" aria-label="下一页">
                                        <span aria-hidden="true">
